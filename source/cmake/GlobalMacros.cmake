...
# DRMDrivers), then put it there instead.

macro(DRMDRIVERS_COMPUTE_SOURCES _framework)
...
macro(DRMDRIVERS_INCLUDE_CONFIG_FILES_IF_EXISTS)
...
macro(DRMDRIVERS_ADD_SOURCE_DEPENDENCIES _source _deps)
    set(_tmp)
...
macro(DRMDRIVERS_ADD_PRECOMPILED_HEADER _header _cpp _source)
    if (MSVC)
        ...
        foreach (_src ${_sources})
            DRMDRIVERS_ADD_SOURCE_DEPENDENCIES(${_src} ${PrecompiledBinary})
        endforeach ()
...
macro(DRMDRIVERS_WRAP_SOURCELIST)
    foreach (_file ${ARGN})
        get_filename_component(_basename ${_file} NAME_WE)
        get_filename_component(_path ${_file} PATH)

        if (NOT _file MATCHES "${DERIVED_SOURCES_DRMDRIVERS_DIR}")
            string(REGEX REPLACE "/" "\\\\\\\\" _sourcegroup "${_path}")
            source_group("${_sourcegroup}" FILES ${_file})
        endif ()
    endforeach ()

    source_group("DerivedSources" REGULAR_EXPRESSION "${DERIVED_SOURCES_DRMDRIVERS_DIR}")
endmacro()

macro(DRMDRIVERS_FRAMEWORK_DECLARE _target)
...
macro(DRMDRIVERS_EXECUTABLE_DECLARE _target)
    add_executable(${_target} "${CMAKE_BINARY_DIR}/cmakeconfig.h")
endmacro()

# Private macro for setting the properties of a target.
# Rather than just having _target like DRMDRIVERS_FRAMEWORK and DRMDRIVERS_EXECUTABLE the parameters are
# split into _target_logical_name, which is used for variable expansion, and _target_cmake_name.
# This is done to support DRMDRIVERS_WRAP_EXECUTABLE which uses the _target_logical_name variables
# but requires a different _target_cmake_name.
macro(_DRMDRIVERS_TARGET _target_logical_name _target_cmake_name)
...
macro(_DRMDRIVERS_TARGET_ANALYZE _target)
    if (ClangTidy_EXE)
...
macro(DRMDRIVERS_FRAMEWORK _target)
    _DRMDRIVERS_TARGET(${_target} ${_target})
    _DRMDRIVERS_TARGET_ANALYZE(${_target})
...
# FIXME Move into DRMDRIVERS_FRAMEWORK after all libraries are using this macro
macro(DRMDRIVERS_FRAMEWORK_TARGET _target)
    ...
    add_library(DRMDrivers::${_target} ALIAS ${_target}_PostBuild)
endmacro()

macro(DRMDRIVERS_EXECUTABLE _target)
    _DRMDRIVERS_TARGET(${_target} ${_target})
    _DRMDRIVERS_TARGET_ANALYZE(${_target})
...
macro(DRMDRIVERS_WRAP_EXECUTABLE _target)
    ...
    _DRMDRIVERS_TARGET(${_target} ${_wrapped_target_name})
    _DRMDRIVERS_TARGET_ANALYZE(${_wrapped_target_name})
...
macro(DRMDRIVERS_CREATE_FORWARDING_HEADER _target_directory _file)
    ...
    # DRMDrivers2 generate-forwarding-headers script.
...
macro(DRMDRIVERS_CREATE_FORWARDING_HEADERS _framework)
    ...
    if (NOT WIN32)
        ...
        foreach (_currentArg ${ARGN})
            ...
            elseif (_processing_directories)
                ...
                foreach (_file ${_files})
                    DRMDRIVERS_CREATE_FORWARDING_HEADER(${_target_directory} ${_file})
                endforeach ()
            elseif (_processing_files)
                DRMDRIVERS_CREATE_FORWARDING_HEADER(${_target_directory} ${_currentArg})
            endif ()
        endforeach ()
    endif ()
endmacro()

function(DRMDRIVERS_MAKE_FORWARDING_HEADERS framework)
...
function(DRMDRIVERS_COPY_FILES target_name)
    set(options FLATTENED)
...
macro(DRMDRIVERS_DEBUG_DUMP_COMMANDS)
    set(CMAKE_VERBOSE_MAKEFILE ON)
endmacro()

macro(DRMDRIVERS_DEBUG_DUMP_VARIABLES)
...
macro(DRMDRIVERS_ADD_TARGET_PROPERTIES _target _property _flags)
...
macro(DRMDRIVERS_POPULATE_LIBRARY_VERSION library_name)
...
macro(DRMDRIVERS_CREATE_SYMLINK target src dest)
